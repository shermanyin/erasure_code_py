...
def gf_mul(a, b):
    ...
    j = i - 255 if i > 254 else i

    return gff_base[j]


"""
Note the different definition of m from ec_base.c
"""
def gf_gen_rs_matrix(m, k):
    mat = []
    gen = 1
...
def print_matrix(m):
    for row in m:
        print ' '.join(map("{:02x}".format,row))


if __name__ == "__main__":
    k = int(raw_input("Enter number of source symbols (k):").strip())
    m = int(raw_input("Enter number of parity symbols (m):").strip())
    
    coef = gf_gen_rs_matrix(m, k)
    print_matrix(coef)
